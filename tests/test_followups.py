...
from tests.conftest import login_as, make_company, make_followup
...
class TestFollowUpList:
    ...
    def test_priority_filter(self, client, regular_user):
        login_as(client, regular_user)
        c = make_company(regular_user, company_name="FU Filter Corp")
    ...
    def test_overdue_filter(self, client, regular_user):
        login_as(client, regular_user)
        c = make_company(regular_user, company_name="Overdue Corp")
        make_followup(
            c, regular_user,
    ...
    def test_completed_filter(self, client, regular_user):
        login_as(client, regular_user)
        c = make_company(regular_user, company_name="Done Corp")
...
class TestFollowUpCreate:
    def test_create_success(self, client, regular_user):
        login_as(client, regular_user)
        c = make_company(regular_user, company_name="New FU Corp")
        resp = client.post(
            "/followups/new",
            data={
                "company_id": c.id,
        ...
        assert resp.status_code == 200
        fu = FollowUp.query.filter_by(company_id=c.id, priority="high").first()
    ...
    def test_missing_company_rejected(self, client, regular_user):
        ...
        resp = client.post(
            "/followups/new",
            data={"company_id": "", "due_date": date.today().isoformat(), "priority": "medium"},
            follow_redirects=True,
        )
        assert resp.status_code == 200
    ...
    def test_ownership_verified(self, client, regular_user, other_user):
        login_as(client, regular_user)
        c = make_company(other_user, company_name="Other FU Corp")
        resp = client.post(
            "/followups/new",
            data={"company_id": c.id, "due_date": date.today().isoformat(), "priority": "medium"},
        )
...
class TestFollowUpEdit:
    def test_edit_success(self, client, regular_user):
        login_as(client, regular_user)
        c = make_company(regular_user, company_name="Edit FU Corp")
...
class TestFollowUpComplete:
    def test_toggle_completes(self, client, regular_user):
        login_as(client, regular_user)
        c = make_company(regular_user, company_name="Toggle Corp")
    ...
    def test_toggle_back(self, client, regular_user):
        login_as(client, regular_user)
        c = make_company(regular_user, company_name="Toggle2 Corp")
    ...
    def test_ajax_complete_returns_json(self, client, regular_user):
        login_as(client, regular_user)
        c = make_company(regular_user, company_name="Ajax Corp")
        fu = make_followup(c, regular_user, completed=False)
        resp = client.post(
...
class TestFollowUpDelete:
    def test_delete_success(self, client, regular_user):
        login_as(client, regular_user)
        c = make_company(regular_user, company_name="Del FU Corp")
...
class TestFollowUpMatrix:
    ...
    def test_matrix_categorises(self, client, regular_user):
        login_as(client, regular_user)
        c = make_company(regular_user, company_name="Matrix Corp")
