...
class TestMeetLinkColumn:
    """meet_link column exists on FollowUp and Interaction."""
    ...
    def test_interaction_meet_link_default_none(self, app, admin_user):
        c = make_company(admin_user)
        interaction = make_interaction(c, admin_user)
        assert interaction.meet_link is None

    def test_interaction_meet_link_can_be_set(self, app, admin_user):
        c = make_company(admin_user)
        interaction = make_interaction(c, admin_user)
        interaction.meet_link = "https://meet.google.com/xyz-uvwx-rst"
        db.session.commit()
        ix2 = db.session.get(Interaction, interaction.id)
        assert ix2.meet_link == "https://meet.google.com/xyz-uvwx-rst"
...
class TestMeetRoutes:
    ...
    def test_create_meet_for_interaction_requires_login(self, client):
        resp = client.post("/google/meet/create-for-interaction/1")
        assert resp.status_code in (302, 401)
